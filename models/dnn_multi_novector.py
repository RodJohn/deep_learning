...
class DeepModel:
    def __init__(self, args, data_dict):
        ...
        self.multi_feats_size = args.multi_feats_size
        self.multi_feats_range = args.multi_feats_range
    ...
    def __init__graph(self):
        ...
        self.cate_feats = tf.placeholder_with_default(self.data_dict['cate_feats'],
                                                      shape=[None, self.cate_field_size + self.multi_feats_size],
                                                      name='cate_feats')
    ...
    def deep(self):
        ...
        embeddings = tf.Variable(tf.random_normal([index_max_size, self.embedding_size], 0.0, 0.01), name='feats_emb')
        self.weights['feats_emb'] = tf.concat((tf.zeros(shape=[1, self.embedding_size]), embeddings[1:, :]), 0)

        with tf.name_scope('deep_part'):
            len_layers = len(self.hidden_units)
            # cate -> Embedding
            self.cate_emb = tf.nn.embedding_lookup(self.weights['feats_emb'],
                                                   ids=self.cate_feats[:, 0:self.cate_field_size])
            self.cate_emb = tf.reshape(self.cate_emb, shape=[-1, self.cate_field_size * self.embedding_size])
            # multi_category -> Embedding
            for item in self.multi_feats_range:
                index_start = int(item[0]) + self.cate_field_size
                index_end = int(item[1]) + self.cate_field_size
                feat_name = item[2]
                ...
                multi_cate_emb = tf.nn.embedding_lookup(self.weights['feats_emb'],
                                                        ids=self.cate_feats[:, index_start:index_end])

                print("mul_cat_emb_lookup:", feat_name, multi_cate_emb)
                axis_2_sum = tf.reduce_sum(multi_cate_emb, axis=2)
                multi_cate_nonzero = tf.count_nonzero(axis_2_sum, 1, keepdims=True, dtype=float)
                multi_cate_sum = tf.reduce_sum(multi_cate_emb, axis=1)
                multi_cate_emb = tf.div_no_nan(multi_cate_sum, multi_cate_nonzero)
                print("mul_cate_reduce_mean:", feat_name, multi_cate_emb)
                if int(item[0]) == 0:
                    self.all_multi_cate_emb = multi_cate_emb
                else:
                    self.all_multi_cate_emb = tf.concat([self.all_multi_cate_emb, multi_cate_emb], axis=1)
                print("all_mul_cate_emb:", self.all_multi_cate_emb)

            # concat continuity & category_embedding & multi_category_embedding -> Dense Vector
            dense_vector = tf.concat([self.cont_feats, self.cate_emb, self.all_multi_cate_emb], axis=1,
                                     name='dense_vector')
            cate_emb_size = self.cate_emb.shape.as_list()[1]
            multi_cate_emb_size = self.all_multi_cate_emb.shape.as_list()[1]
            input_size = self.cont_field_size + cate_emb_size + multi_cate_emb_size
            print("cont_size = ", self.cont_field_size)
            print("cate_emb_size = ", cate_emb_size)
    ...
    def get_val_data(sess, data):
        ...
        try:
            while True:
                cont_value, cate_index, label = sess.run(
                    [
                        data["cont_feats"],
                        data["cate_feats"],
                        data["label"]
