...
Feature:
1.vector
2.cate(单值离散: 原始离散特征及其他(例如：连续特征离散化)
"""
...
class DeepModel:
    ...
    def deep_fm(self):
        ...
        with tf.name_scope('fm_part'):
            input_feats_index = self.cate_feats
            input_feats_value = self.cate_feats_value
            input_feats_field_size = self.cate_field_size
        ...
        with tf.name_scope('deep_part'):
            ...
            dense_vector = tf.concat([self.vector_feats, cat_emb], axis=1,
                                     name='dense_vector')

            cat_size = cat_emb.shape.as_list()[1]
            input_size = self.vector_field_size + cat_size
            print("cate_emb size = ", cat_size)
