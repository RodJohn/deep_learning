...
class DeepModel:
    ...
    def deep_fm(self):
        index_max_size = self.cate_index_size + self.cont_field_size
        ...
        with tf.name_scope('fm_part'):
            input_field_size = self.cont_field_size + self.single_cate_field_size + self.multi_cate_field_size
            cont_index_add = tf.add(self.cont_feats_index, self.cate_index_size)

            # FM_first_order [?, input_field_size]
            # cont
            first_cont_emb = tf.nn.embedding_lookup(self.weights['fm_first_order_emb'], ids=cont_index_add)
            first_cont_emb = tf.reshape(first_cont_emb, shape=[-1, self.cont_field_size])
            first_cont_mul = tf.multiply(first_cont_emb, self.cont_feats)
            # single_cate
            ...
            first_order_emb = tf.concat([first_cont_mul, first_single_cate_emb, self.all_first_multi_emb], axis=1)
            ...
            # cont
            second_cont_emb = tf.nn.embedding_lookup(self.weights['feats_emb'], ids=cont_index_add)
            second_cont_value = tf.reshape(self.cont_feats, shape=[-1, self.cont_field_size, 1])
        ...
        with tf.name_scope('deep_part'):
            ...
            # concat continuity & vector & single_category_embedding & multi_category_embedding -> Dense Vector
            dense_vector = tf.concat([self.cont_feats, self.vector_feats, self.single_cate_emb,
            ...
            input_size = self.cont_field_size + self.vector_feats_size + single_cate_emb_size + multi_cate_emb_size
