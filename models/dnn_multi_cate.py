...
class DeepModel:
    ...
    def deep(self):
        ...
        self.weights['feats_emb'] = tf.concat((tf.zeros(shape=[1, self.embedding_size]), self.weights['feats_emb'][1:]),
                                              0)
        ...
        with tf.name_scope('deep_part'):
            ...
            # concat vector & category_embedding & multi_category_embedding -> Dense Vector
            dense_vector = tf.concat([self.vector_feats, self.cate_emb, self.all_multi_cate_emb], axis=1,
                                     name='dense_vector')
            cate_emb_size = self.cate_emb.shape.as_list()[1]
            multi_cate_emb_size = self.all_multi_cate_emb.shape.as_list()[1]
            input_size = self.vector_feats_size + cate_emb_size + multi_cate_emb_size
        ...
        try:
            while True:
                ...
                feed_dict_map = {
                    ...
                    "input_data_size:0": [len(cate_index)]
                }
